...
app.controller("CalendarController", function ($scope, $http) {
  ...
  calendar.previousMonth = function () {
    ...
    calendar.loadData(); // Add this line
  ...
  calendar.generateWeeks = function () {
    ...
    for (var i = 1; i <= numDays; i++) {
      ...
      var isReserved = calendar.reservedNights.find(
        (night) => night.time === dateTime
      );
  ...
  calendar.removeInitData = function (dateTime) {
    $http
      ...
      .then(function (response) {
        var initData = response.data;
        initData.data = initData.data.filter(
          (night) => night.time !== dateTime
        );
